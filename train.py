    ...
    with torch.no_grad():
        for data in dataloader:
            ...
            if supervised:
                ...
                pred_c = torch.mean(y_source, dim=2)
                pred_s = torch.mean(y_source, dim=4)
                loss = (criterion(pred_c, target_c) + criterion(pred_s, target_s)) / 2.0
...
def main():
    ...
    # Data arguments
    parser.add_argument('--data_dir', type=str, default='/srv/shared/data/combat/train',
    ...
    parser.add_argument('--architecture', type=str, default='original_mri',
                        choices=['original_mri', 'original_mri_3d', 'simple_3d'],
    ...
    # Setup model
    model = networks.Proj2VolRegistration(
