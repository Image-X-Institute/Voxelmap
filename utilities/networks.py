...
class Proj2VolRegistration(LoadableModel):
    """
    Projection-to-volume registration network.
    
    Architecture variants:
    - 'original_mri': Separate 2D encoders for source/target projections, 2D->3D transform
    - 'original_mri_3d': 2D encoder for target projections, 3D encoder for source volume
    ...
    def __init__(self, im_size, architecture='original_mri', int_steps=7, skip_connections=False):
        ...
        elif architecture == 'original_mri_3d':
            self._setup_original_mri_3d(enc_nf)
    ...
    def _setup_original_mri_3d(self, enc_nf):
        """Original MRI variant: 2D encoder for target projections, 3D encoder for source volume"""
        self.encoder_c = Encoder2D(1, enc_nf)  # target coronal
        self.encoder_s = Encoder2D(1, enc_nf)  # target sagittal
        self.encoder_vol = Encoder3D(1, enc_nf)  # source volume
        
        # Transform block for 2D features
        self.transform = TransBlock2Dto3D()
        
        # 3D decoder - starts with 2*enc_nf[-1] (2D features) + enc_nf[-1] (3D features)
        self.decoder = Decoder3D(enc_nf, dec_start_channels=3*enc_nf[-1], out_channels=3, use_skip_connections=self.skip_connections)
    ...
    def _setup_simple_3d(self, enc_nf):
        ...
        self.decoder = Decoder3D(enc_nf, dec_start_channels=enc_nf[-1], out_channels=3, use_skip_connections=self.skip_connections)
    ...
    def forward(self, source_c, source_s, target_c, target_s, source_vol):
        ...
        elif self.architecture == 'original_mri_3d':
            return self._forward_original_mri_3d(source_c, source_s, target_c, target_s, source_vol)
    ...
    def _forward_original_mri_3d(self, source_c, source_s, target_c, target_s, source_vol):
        """Original MRI 3D architecture forward pass"""
        # Encode target projections in 2D
        enc_c = self.encoder_c(target_c)
        enc_s = self.encoder_s(target_s)
        
        # Encode source volume in 3D
